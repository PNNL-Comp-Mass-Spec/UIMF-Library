...
namespace UIMFLibrary.UnitTests.DataReaderTests
{
	...
	public class DataReaderTests
	{
		...
		public void displayMZValueForEachBin_Test1()
		{
			...
			using (this.m_reader = new DataReader(filePath))
			{
				GlobalParameters gp = this.m_reader.GetGlobalParameters();
				FrameParameters fp = this.m_reader.GetFrameParameters(testFrame);
				...
				for (int i = 0; i < 400000; i++)
				{
					...
					sb.Append('\t');
					double mz =
						(double)this.convertBinToMZ(fp.CalibrationSlope, fp.CalibrationIntercept, gp.BinWidth, gp.TOFCorrectionTime, i);
